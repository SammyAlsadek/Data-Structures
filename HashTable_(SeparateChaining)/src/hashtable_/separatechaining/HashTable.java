...
class Entry<K, V> {
	...
	K key;
	V value; // key and value of the entry
	...
	public boolean equals(Entry<K, V> other) {
		...
		if (this.hash != other.hash)
			return false; // if hashes are not equal return false
...
public class HashTable<K, V> {
	...
	private V bucketInsertEntry(int bucketIndex, Entry<K, V> entry) {
		...
		if (existentEntry == null) {
			...
			return null; // indicates that there was no previous entry
	...
	private Entry<K, V> bucketSeekEntry(int bucketIndex, K key) {
		...
		if (key == null)
			return null; // returns null is key is null
		
		LinkedList<Entry<K, V>> bucket = table[bucketIndex];
		
		if (bucket == null)
			return null; // returns null if the bucket is empty
	...
	// resizes the table holding buckets of entries
	@SuppressWarnings("unchecked")
