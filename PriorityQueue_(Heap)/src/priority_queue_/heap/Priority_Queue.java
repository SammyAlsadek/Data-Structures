...
public class Priority_Queue<T extends Comparable<T>> {
    ...
    // clears the heap
    public void clear() {
        for (int i = 0; i < this.heapCapacity; i++) heap.set(i, null);
        this.heapSize = 0;
        map.clear();
    }
    ...
    public T pull() {return removeAt(0);}
    ...
    private boolean less (int i, int j) {return heap.get(i).compareTo(heap.get(j)) <= 0;}
    ...
    private void sink(int nodeNumber) {
        ...
        while (true) {
            ...
            if (rightNode < this.heapSize && less(rightNode, leftNode)) smallest = rightNode;
            
            // stop if we're outside the bounds of the tree
            // or stop if we cant sink anymore
            if (leftNode >= this.heapSize || less(nodeNumber, smallest)) break;
